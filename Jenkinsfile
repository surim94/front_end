...
def DOCKER_IMAGE='template/happybus_img'
...
podTemplate(label:label,
    ...
    node(label) {
        ...
        stage('DEPLOY') {
            container('kubectl') {
                kubeCmd.apply file: 'k8s/happybus-ingress.yaml', namespace: K8S_NAMESPACE
                kubeCmd.apply file: 'k8s/happybus-service.yaml', namespace: K8S_NAMESPACE
                yaml.update file: 'k8s/happybus-deployment.yaml', update: ['.spec.template.spec.containers[0].image': "${HARBOR_REGISTRY}/${DOCKER_IMAGE}:${BUILD_NUMBER}"]
                kubeCmd.apply file: 'k8s/happybus-deployment.yaml', wait: 1000, recoverOnFail: false, namespace: K8S_NAMESPACE
            }
        }
    }
}
